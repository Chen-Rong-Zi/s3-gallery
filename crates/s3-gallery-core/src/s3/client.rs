...
pub trait S3Client: Send + Sync + 'static {
    ...
    /// # Errors
    ///
    /// Returns `S3GalleryError::ObjectNotFound` if the key does not exist.
    ...
    /// # Errors
    ///
    /// Returns `S3GalleryError::ObjectNotFound` if the key does not exist, or
    /// `S3GalleryError::S3Error` if the range is invalid.

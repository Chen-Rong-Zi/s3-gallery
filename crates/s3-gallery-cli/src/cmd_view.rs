...
pub async fn run_view(cli: &Cli, _host: &str, view_cmd: &ViewCommands) -> Result<()> {
    ...
    match view_cmd {
        ViewCommands::Ls {
            ...
            let entries = view.list_directory(_host, prefix, sort, order).await?;
        ...
        ViewCommands::Tree { path } => {
            let prefix = path.as_deref().unwrap_or("");
            let tree = view.build_tree(_host, prefix).await?;
            print_tree(&tree, 0);
        }
        ViewCommands::Stat { path: _ } => {
            let stats = view.get_stats(_host).await?;
        ...
        ViewCommands::Search { query } => {
            let results = view.search_by_name(_host, query).await?;
        ...
        ViewCommands::Duplicates => {
            let groups = view.find_duplicates(_host).await?;
            if groups.is_empty() {
        ...
        ViewCommands::Timeline => {
            let timeline = view.get_timeline(_host).await?;
            for entry in &timeline {
                println!("{} ({} files)", entry.date, entry.count);
            }
        }
        ViewCommands::Tags { tag_command } => match tag_command {
            TagCommands::List => {
            ...
            TagCommands::Files { tag } => {
                let files = view.get_files_by_tag(_host, tag).await?;
                for file in &files {
                    println!("{}", file.key);
                }
            }
        ...
        ViewCommands::Diff { diff_command } => match diff_command {
        ...
        ViewCommands::Export { format } => {
            ...
            let output = view.export_files(_host, fmt).await?;
            println!("{}", output);
        }
        ViewCommands::Schema => {
...
fn print_tree(node: &s3_gallery_core::view::tree::TreeNode, depth: usize) {

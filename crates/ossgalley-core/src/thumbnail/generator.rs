...
///
/// # Errors
/// Returns `OssgalleyError::ThumbnailGeneration` if the image fails to decode or encode.
pub fn generate_thumbnail(data: &[u8]) -> Result<Vec<u8>> {
    let img = image::load_from_memory(data)
        .map_err(|e| OssgalleyError::ThumbnailGeneration(format!("Failed to decode image: {}", e)))?;
    ...
    thumbnail.write_to(&mut cursor, ImageFormat::Jpeg)
        .map_err(|e| OssgalleyError::ThumbnailGeneration(format!("Failed to encode JPEG: {}", e)))?;
...
impl ThumbnailCache {
    ...
    ///
    /// # Errors
    /// Returns `OssgalleyError` if the cache lookup fails or thumbnail generation fails.
    ...
    ///
    /// # Errors
    /// Returns `OssgalleyError` if the database lookup fails.
    pub async fn get_cached(&self, key: &ObjectKey) -> Result<Option<ThumbnailEntry>> {
        match ThumbnailEntry::get(&self.db, key.as_str()).await {
            ...
            Err(OssgalleyError::NotFound(_)) => Ok(None),
            Err(e) => Err(e),
        }
    }
    ...
    ///
    /// # Errors
    /// Returns `OssgalleyError` if the database insert fails.

//! Database models and CRUD operations for the ossgalley core library.
//!
...
impl HostConfigEntry {
    ...
    /// # Errors
    ///
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn insert(pool: &SqlitePool, entry: &HostConfigEntry) -> Result<()> {
        sqlx::query(
            "INSERT INTO host_config (host_id, host_name, host_type, description, created_at) \
             VALUES (?, ?, ?, ?, ?)",
        )
        .bind(&entry.host_id)
        ...
        .execute(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to insert host config: {e}")))?;
        Ok(())
    }
    ...
    /// # Errors
    ///
    /// Returns `OssgalleyError::NotFound` if no entry with the given id exists.
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn get(pool: &SqlitePool, host_id: &str) -> Result<HostConfigEntry> {
        sqlx::query_as::<_, HostConfigEntry>("SELECT * FROM host_config WHERE host_id = ?")
            ...
            .fetch_optional(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to get host config: {e}")))?
            .ok_or_else(|| OssgalleyError::NotFound(format!("Host config not found: {host_id}")))
    }
    ...
    pub async fn update(pool: &SqlitePool, entry: &HostConfigEntry) -> Result<()> {
        sqlx::query(
            "UPDATE host_config SET host_name = ?, host_type = ?, description = ?, created_at = ? \
             WHERE host_id = ?",
        )
        .bind(&entry.host_name)
        .bind(&entry.host_type)
        ...
        .execute(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to update host config: {e}")))?;
        Ok(())
    }
    ...
    pub async fn delete(pool: &SqlitePool, host_id: &str) -> Result<()> {
        sqlx::query("DELETE FROM host_config WHERE host_id = ?")
            ...
            .execute(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to delete host config: {e}")))?;
        Ok(())
    }
}
...
impl FileEntry {
    ...
    pub async fn insert(pool: &SqlitePool, entry: &FileEntry) -> Result<()> {
        sqlx::query(
            "INSERT INTO files (key, etag, size, last_modified, content_type, file_type, \
        ...
        .execute(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to insert file: {e}")))?;
        Ok(())
    }
    ...
    pub async fn upsert(pool: &SqlitePool, entry: &FileEntry) -> Result<()> {
        sqlx::query(
            "INSERT OR REPLACE INTO files (key, etag, size, last_modified, content_type, file_type, \
             metadata_state, is_deleted) \
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&entry.key)
        ...
        .execute(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to upsert file: {e}")))?;
        Ok(())
    }

    /// Get a file entry by its key.
    ///
    /// # Errors
    ///
    /// Returns `OssgalleyError::NotFound` if no file with the given key exists.
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn get_by_key(pool: &SqlitePool, key: &str) -> Result<FileEntry> {
        sqlx::query_as::<_, FileEntry>("SELECT * FROM files WHERE key = ?")
            .bind(key)
            .fetch_optional(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to get file: {e}")))?
            .ok_or_else(|| OssgalleyError::NotFound(format!("File not found: {key}")))
    }

    /// List files whose key starts with the given prefix.
    ///
    ...
    ///
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn list_by_prefix(pool: &SqlitePool, prefix: &str) -> Result<Vec<FileEntry>> {
        sqlx::query_as::<_, FileEntry>(
            "SELECT * FROM files WHERE key LIKE ? || '%' AND is_deleted = 0 ORDER BY key",
        )
        .bind(prefix)
        .fetch_all(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to list files by prefix: {e}")))
    }

    /// List files with the given file type.
    ///
    /// Only non-deleted files are returned, ordered by key.
    ///
    /// # Errors
    ///
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn list_by_file_type(pool: &SqlitePool, file_type: &str) -> Result<Vec<FileEntry>> {
        sqlx::query_as::<_, FileEntry>(
            "SELECT * FROM files WHERE file_type = ? AND is_deleted = 0 ORDER BY key",
        )
        .bind(file_type)
        .fetch_all(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to list files by type: {e}")))
    }
    ...
    ///
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn mark_deleted(pool: &SqlitePool, key: &str) -> Result<()> {
        sqlx::query("UPDATE files SET is_deleted = 1 WHERE key = ?")
            .bind(key)
            .execute(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to mark file deleted: {e}")))?;
        Ok(())
    }
    ...
    pub async fn count(pool: &SqlitePool) -> Result<i64> {
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM files WHERE is_deleted = 0")
            .fetch_one(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to count files: {e}")))
    }
}
...
impl MetadataEntry {
    ...
    pub async fn insert(pool: &SqlitePool, entry: &MetadataEntry) -> Result<()> {
        ...
        .execute(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to insert metadata: {e}")))?;
        Ok(())
    }
    ...
    pub async fn get_by_file_key(pool: &SqlitePool, file_key: &str) -> Result<Vec<MetadataEntry>> {
        ...
        .fetch_all(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to get metadata: {e}")))
    }
    ...
    pub async fn get_by_namespace(
        ...
        .fetch_all(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to get metadata by namespace: {e}")))
    }
    ...
    pub async fn delete_by_file_key(pool: &SqlitePool, file_key: &str) -> Result<()> {
        sqlx::query("DELETE FROM metadata WHERE file_key = ?")
            ...
            .execute(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to delete metadata: {e}")))?;
        Ok(())
    }
}
...
impl ThumbnailEntry {
    ...
    pub async fn insert(pool: &SqlitePool, entry: &ThumbnailEntry) -> Result<()> {
        ...
        .execute(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to insert thumbnail: {e}")))?;
        Ok(())
    }
    ...
    /// # Errors
    ///
    /// Returns `OssgalleyError::NotFound` if no thumbnail with the given key exists.
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn get(pool: &SqlitePool, file_key: &str) -> Result<ThumbnailEntry> {
        sqlx::query_as::<_, ThumbnailEntry>("SELECT * FROM thumbnails WHERE file_key = ?")
            ...
            .fetch_optional(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to get thumbnail: {e}")))?
            .ok_or_else(|| OssgalleyError::NotFound(format!("Thumbnail not found: {file_key}")))
    }
    ...
    pub async fn update(pool: &SqlitePool, entry: &ThumbnailEntry) -> Result<()> {
        ...
        .execute(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to update thumbnail: {e}")))?;
        Ok(())
    }
    ...
    pub async fn delete(pool: &SqlitePool, file_key: &str) -> Result<()> {
        sqlx::query("DELETE FROM thumbnails WHERE file_key = ?")
            ...
            .execute(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to delete thumbnail: {e}")))?;
        Ok(())
    }
}
...
impl TagEntry {
    ...
    pub async fn insert(pool: &SqlitePool, entry: &TagEntry) -> Result<()> {
        sqlx::query("INSERT INTO tags (tag_name, tag_type) VALUES (?, ?)")
            ...
            .execute(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to insert tag: {e}")))?;
        Ok(())
    }
    ...
    /// # Errors
    ///
    /// Returns `OssgalleyError::NotFound` if no tag with the given name exists.
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn get_by_name(pool: &SqlitePool, tag_name: &str) -> Result<TagEntry> {
        sqlx::query_as::<_, TagEntry>("SELECT * FROM tags WHERE tag_name = ?")
            ...
            .fetch_optional(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to get tag: {e}")))?
            .ok_or_else(|| OssgalleyError::NotFound(format!("Tag not found: {tag_name}")))
    }
    ...
    pub async fn list_all(pool: &SqlitePool) -> Result<Vec<TagEntry>> {
        sqlx::query_as::<_, TagEntry>("SELECT * FROM tags ORDER BY tag_name")
            .fetch_all(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to list tags: {e}")))
    }
}
...
impl FileTagEntry {
    ...
    pub async fn insert(pool: &SqlitePool, entry: &FileTagEntry) -> Result<()> {
        sqlx::query("INSERT INTO file_tags (file_key, tag_id) VALUES (?, ?)")
            ...
            .execute(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to insert file tag: {e}")))?;
        Ok(())
    }
    ...
    pub async fn get_by_file_key(pool: &SqlitePool, file_key: &str) -> Result<Vec<FileTagEntry>> {
        sqlx::query_as::<_, FileTagEntry>("SELECT * FROM file_tags WHERE file_key = ?")
            ...
            .fetch_all(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to get file tags: {e}")))
    }
    ...
    pub async fn delete(pool: &SqlitePool, file_key: &str, tag_id: i64) -> Result<()> {
        sqlx::query("DELETE FROM file_tags WHERE file_key = ? AND tag_id = ?")
            ...
            .execute(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to delete file tag: {e}")))?;
        Ok(())
    }
}
...
/// A row in the `scan_metadata` table (singleton row).
#[derive(Debug, Clone, sqlx::FromRow)]
...
impl ScanMetadata {
    /// Get the singleton scan metadata row.
    ///
    /// # Errors
    ///
    /// Returns `OssgalleyError::NotFound` if no scan metadata row exists.
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn get(pool: &SqlitePool) -> Result<ScanMetadata> {
        sqlx::query_as::<_, ScanMetadata>("SELECT * FROM scan_metadata LIMIT 1")
            .fetch_optional(pool)
            .await
            .map_err(|e| OssgalleyError::DbError(format!("Failed to get scan metadata: {e}")))?
            .ok_or_else(|| OssgalleyError::NotFound("Scan metadata not found".to_string()))
    }

    /// Update the singleton scan metadata row.
    ///
    /// # Errors
    ///
    /// Returns `OssgalleyError::DbError` if the database operation fails.
    pub async fn update(pool: &SqlitePool, metadata: &ScanMetadata) -> Result<()> {
        ...
        .execute(pool)
        .await
        .map_err(|e| OssgalleyError::DbError(format!("Failed to update scan metadata: {e}")))?;
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_file_entry_crud() -> Result<()> {
        ...
        // Read back
        let fetched = FileEntry::get_by_key(&pool, "test/file.jpg").await?;
        ...
        // Mark as deleted
        FileEntry::mark_deleted(&pool, "test/file.jpg").await?;
        let fetched = FileEntry::get_by_key(&pool, "test/file.jpg").await?;
    ...
    async fn test_file_entry_list_by_prefix() -> Result<()> {
        ...
        let results = FileEntry::list_by_prefix(&pool, "photos/").await?;
        assert_eq!(results.len(), 2);
        assert_eq!(results[0].key, "photos/vacation/img001.jpg");
        assert_eq!(results[1].key, "photos/vacation/img002.jpg");

        // List by prefix "docs/"
        let results = FileEntry::list_by_prefix(&pool, "docs/").await?;
    ...
    async fn test_file_entry_list_by_file_type() -> Result<()> {
        ...
        let results = FileEntry::list_by_file_type(&pool, "pdf").await?;
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].key, "doc.pdf");

        let results = FileEntry::list_by_file_type(&pool, "png").await?;
        assert!(results.is_empty());
    ...
    async fn test_file_entry_count() -> Result<()> {
        ...
        FileEntry::mark_deleted(&pool, "test.txt").await?;
        let count = FileEntry::count(&pool).await?;
        assert_eq!(count, 0);
    ...
    async fn test_scan_metadata_crud() -> Result<()> {
        ...
        // The row should exist after migration with default schema version

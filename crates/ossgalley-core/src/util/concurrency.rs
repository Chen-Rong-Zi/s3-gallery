...
impl ConcurrencyLimiter {
        ...
        F: Future<Output = Result<T>>,
    {
        let _permit = self.semaphore
            .acquire()
            .await
            .map_err(|e| OssgalleyError::Internal(format!("Failed to acquire semaphore permit: {}", e)))?;
...
mod tests {
    ...
    async fn test_execute_returns_result() -> Result<()> {
        ...
        let result = limiter.execute(async { Ok::<_, OssgalleyError>(42) }).await?;
        assert_eq!(result, 42);
        Ok(())
    }
    ...
    async fn test_concurrency_limit_respected() -> Result<()> {
        ...
        handle.await.map_err(|e| OssgalleyError::Internal(e.to_string()))??;
        assert_eq!(limiter.available_permits(), 2);
        Ok(())
    }

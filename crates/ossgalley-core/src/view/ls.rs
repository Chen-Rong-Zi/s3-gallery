...
pub async fn list_directory(
    ...
    let files = FileEntry::list_by_prefix(db, prefix).await?;
    ...
    for dir in directories {
        entries.push(LsEntry::directory(dir));
    }
...
mod tests {
    ...
    async fn test_list_directory_root() -> Result<()> {
        ...
        let entries = list_directory(&pool, "", SortField::Name, SortOrder::Ascending).await?;
    ...
    async fn test_list_directory_photos_2024() -> Result<()> {
        ...
        let entries = list_directory(&pool, "photos/2024", SortField::Name, SortOrder::Ascending)
            .await?;
        assert_eq!(entries.len(), 2);
    ...
    async fn test_list_directory_deleted_excluded() -> Result<()> {
        ...
        let entries = list_directory(&pool, "docs", SortField::Name, SortOrder::Ascending).await?;

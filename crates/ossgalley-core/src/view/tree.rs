...
pub async fn build_tree(db: &SqlitePool, root_prefix: &str) -> Result<TreeNode> {
    let files = FileEntry::list_by_prefix(db, root_prefix).await?;
    ...
    fn build_node(
        ...
        let data = node_map.get(path).ok_or_else(|| {
            crate::error::OssgalleyError::Internal("Node not found".into())
        })?;
...
mod tests {
    ...
    async fn test_build_tree() -> Result<()> {
        ...
        let tree = build_tree(&pool, "").await?;
    ...
    async fn test_build_tree_subdir() -> Result<()> {
        ...
        let tree = build_tree(&pool, "photos").await?;

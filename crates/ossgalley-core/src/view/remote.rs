...
#[cfg(test)] use crate::error::OssgalleyError;
use crate::types::*;
...
impl RemoteView {
    ...
    pub async fn fetch_file_content(&self, key: &ObjectKey) -> Result<Vec<u8>> {
        self.s3.get_object(&self.bucket, key).await
    }
    ...
    pub async fn fetch_thumbnail(&self, key: &ObjectKey) -> Result<Vec<u8>> {
        ...
        // Cache locally
        ThumbnailEntry::insert(&self.db, &ThumbnailEntry {
    ...
    pub async fn fetch_byte_range(&self, key: &ObjectKey, start: u64, end: u64) -> Result<Vec<u8>> {
        self.s3.get_object_range(&self.bucket, key, start, end).await
    }
...
mod tests {
    ...
    fn setup_test_key(name: &str) -> ObjectKey {
        ObjectKey::new(name).ok().unwrap_or_else(|| ObjectKey::new("a").ok().unwrap_or_else(|| loop {}))
    }

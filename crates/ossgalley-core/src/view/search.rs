...
pub async fn search_by_name(db: &SqlitePool, query: &str) -> Result<SearchResult> {
...
///
/// Returns an error if the database query fails.
pub async fn search_by_tag(db: &SqlitePool, tag_name: &str) -> Result<SearchResult> {
    let files: Vec<FileEntry> = sqlx::query_as(
...
mod tests {
    ...
    async fn test_search_by_name() -> Result<()> {
        ...
        let result = search_by_name(&pool, "jpg").await?;
    ...
    async fn test_search_by_name_partial() -> Result<()> {
        ...
        let result = search_by_name(&pool, "vacation").await?;
    ...
    async fn test_search_by_name_no_match() -> Result<()> {
        ...
        let result = search_by_name(&pool, "nonexistent").await?;
    ...
    async fn test_search_by_tag() -> Result<()> {
        ...
        let result = search_by_tag(&pool, "photo").await?;
        assert_eq!(result.total_count, 2);
    ...
    async fn test_search_by_tag_no_match() -> Result<()> {
        ...
        let result = search_by_tag(&pool, "nonexistent").await?;
        assert_eq!(result.total_count, 0);
        assert!(result.files.is_empty());

...
impl MockS3Client {
    ...
    /// # Errors
    ///
    /// Returns `OssgalleyError::ValidationError` if any key is invalid.
    ...
    /// # Errors
    ///
    /// Returns `OssgalleyError::ValidationError` if the key cannot be
...
impl S3Client for MockS3Client {
    ...
    async fn head_object(
        ...
        let obj = storage
            .get(key.as_str())
            .ok_or_else(|| OssgalleyError::ObjectNotFound(key.to_string()))?;
    ...
    async fn get_object_range(
        ...
        let start_us = usize::try_from(start)
            .map_err(|e| OssgalleyError::S3Error(format!("invalid start offset: {e}")))?;
        let end_us = usize::try_from(end)
            .map_err(|e| OssgalleyError::S3Error(format!("invalid end offset: {e}")))?;

        let slice = obj
            .body
            .get(start_us..end_us)
            .ok_or_else(|| OssgalleyError::S3Error("invalid byte range".to_string()))?;
...
mod tests {
    ...
    async fn test_list_objects_with_prefix() -> Result<()> {
        ...
        for summary in &results {
            ...
            assert!(key_str.starts_with("photos/2024/"), "unexpected key: {key_str}");
        }
        Ok(())
    }
    ...
    async fn test_object_not_found() -> Result<()> {
        ...
        assert!(matches!(err, OssgalleyError::ObjectNotFound(_)));
    ...
    async fn test_with_fixtures() -> Result<()> {
        let client = MockS3Client::with_fixtures(vec![
            ("a.txt", b"content a"),
            ("b.txt", b"content b"),
        ])?;
        let bucket = BucketName::new("test-bucket")?;

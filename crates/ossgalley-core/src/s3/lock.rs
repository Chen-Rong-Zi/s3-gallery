...
impl LockGuard {
    ...
    pub(super) fn new(
        ...
        Self { s3, bucket, lock_key, client_id }
    }
    ...
    /// # Errors
    ///
    /// Returns `OssgalleyError::S3Error` if the S3 delete operation fails.
    ...
    /// # Errors
    ///
    /// Returns `OssgalleyError::Internal` if the lease fails to serialize.
    /// Returns `OssgalleyError::S3Error` if the S3 put operation fails.
    pub async fn renew(&mut self) -> Result<()> {
        let lease = LockLease::new(self.client_id.clone());
        let body = serde_json::to_vec(&lease)
            .map_err(|e| OssgalleyError::Internal(format!("Failed to serialize lease: {}", e)))?;
        self.s3.put_object(&self.bucket, &self.lock_key, &body).await?;
        Ok(())
    }
}
...
/// Returns `OssgalleyError::Internal` if the lease fails to serialize.
/// Returns `OssgalleyError::S3Error` if the S3 put operation fails.
/// Returns `OssgalleyError::LockContention` if the lock is already held by another client.
pub async fn acquire_lock(
    ...
    let acquired = s3.put_object_if_none_match(&bucket, &lock_key, &body).await?;

    if acquired {
        Ok(LockGuard::new(s3, bucket, lock_key, client_id))
    } else {
        Err(OssgalleyError::LockContention("Lock is held by another client".to_string()))
    }
}
...
mod tests {
    ...
    fn test_key(name: &str) -> ObjectKey {
        ObjectKey::new(name).ok().unwrap_or_else(||
            ObjectKey::new("test-key").ok().unwrap_or_else(|| loop {})
        )
    }

    fn test_bucket(name: &str) -> BucketName {
        BucketName::new(name).ok().unwrap_or_else(||
            BucketName::new("test-bucket").ok().unwrap_or_else(|| loop {})
        )
    }
    ...
    async fn test_acquire_and_release_lock() -> Result<()> {
        ...
        let lock_key = test_key("test-host/.ossgallery/db.lock");
    ...
    async fn test_lock_contention() -> Result<()> {
        ...
        let _guard = acquire_lock(s3.clone(), bucket.clone(), lock_key.clone(), "client-1".to_string()).await?;

        let result = acquire_lock(s3.clone(), bucket.clone(), lock_key.clone(), "client-2".to_string()).await;
    ...
    async fn test_renew_lock() -> Result<()> {
        ...
        let mut guard = acquire_lock(s3.clone(), bucket.clone(), lock_key.clone(), "client-1".to_string()).await?;
        guard.renew().await?;
        guard.release().await?;
        Ok(())
    }
    ...
    async fn test_acquire_lock_after_release() -> Result<()> {
        ...
        let guard = acquire_lock(s3.clone(), bucket.clone(), lock_key.clone(), "client-1".to_string()).await?;
        guard.release().await?;

        // Acquire again should succeed
        let guard2 = acquire_lock(s3.clone(), bucket.clone(), lock_key.clone(), "client-2".to_string()).await?;
